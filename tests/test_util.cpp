...
BOOST_AUTO_TEST_SUITE( Testing_generateRandomLabels )
    ...
    auto output = util::generateRandomLabels(128, globalDelta, inputWiresLabels, outputWiresLabels);
    vector<uint64_t> output0 = get<0>(output);
    vector<tuple<vector<uint64_t>, vector<uint64_t>>> output1 = get<1>(output);
    vector<tuple<vector<uint64_t>, vector<uint64_t>>> output2 = get<2>(output);
